...
uint8_t values[numChannels];      // the array to store summary of signal counts per channel
...
Adafruit_SH1106G display = Adafruit_SH1106G(SCREEN_WIDTH, SCREEN_HEIGHT, &SPI, TFT_DC, TFT_RST, TFT_CS);
...
Serial.println("BUSIO_HAS_HW_SPI true");
#else
Serial.println("BUSIO_HAS_HW_SPI false");
#endif

#if defined(SPI_INTERFACES_COUNT)
Serial.print("SPI_INTERFACES_COUNT=");
Serial.println(SPI_INTERFACES_COUNT);
#else
Serial.println("SPI_INTERFACES_COUNT false");
#endif
...
void loop(void) {
  ...
  for (int i = 0; i < numChannels; ++i) {
    uint8_t clamped = min(0xf, values[i]);
    uint16_t barHeight = chartHeight * clamped;
    barHeight /= 0xf;

    uint8_t x = (barWidth * i) + 1 + margin - (barWidth * (bool)i);
    display.fillRect(x, 0, barWidth, chartHeight, BLACK);
    display.fillRect(x, chartHeight - barHeight, barWidth, barHeight, WHITE);

    if (values[i])
      Serial.print(clamped, HEX);
    else
      Serial.print(F("-"));
  }
  Serial.println();
...
//       Serial.print(channel);
//       Serial.print(", cacheSum=");
//       Serial.println(cacheSum);
//     }
